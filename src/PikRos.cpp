...
//-------------------IkTask-------------------
IkTask::IkTask(IkTaskType t_type, const std::string& t_frame_name, const Eigen::VectorXd& t_desired_value,
               ApproachAxis t_approach_axis)
...
double Gradient::getGradient() const
{
  return gradient_;
};

//-------------------Pik-------------------
Pik::Pik(ros::NodeHandle& nh, const std::string& robot_name, const std::string& joint_model_group_name,
         const Settings& settings)
  : moveit_wrapper_(nh, robot_name, joint_model_group_name), settings_(settings), n_dof_(moveit_wrapper_.getNDof())
{
}

Eigen::VectorXd Pik::solve(const std::vector<IkTask>& ik_tasks, const Eigen::VectorXd& q_initial_guess)
{
  // initialize solver
  ...
  while (!reachedStoppingCriteria())
  {
    ...
    {
      if (settings_.debug_mode)
        std::cout << "Entered polish solution!\n";
...
Eigen::VectorXd Pik::getTaskError(const Eigen::VectorXd& q, IkTask task, bool clamp_err) const
{
  if (task.type == FRAME_POSE)
  {
    ...
    Eigen::Vector3d ori_err =
        getOrientationErrorRPY(desired_quaternion, vec2quat(getTaskValue(q, task).segment(3, 4)), clamp_err);
  ...
  if (task.type == FRAME_ORIENTATION)
  {
    return getOrientationErrorRPY(vec2quat(task.desired_value), vec2quat(getTaskValue(q, task)), clamp_err);
  }
  if (task.type == FRAME_APPROACH_AXIS)
  {
    Eigen::Quaterniond tool_quaternion = vec2quat(getTaskValue(q, task));
...
Eigen::Vector3d Pik::getOrientationErrorRPY(Eigen::Quaterniond desired_quat, Eigen::Quaterniond current_quat,
                                            bool clamp_err) const
{
  ...
  Eigen::Vector3d axis(quaternion_err.x() / sin_angle, quaternion_err.y() / sin_angle, quaternion_err.z() / sin_angle);
...
Eigen::VectorXd Pik::solvePtscProblem(const std::vector<PtscEigen::Task>& ptsc_tasks) const
{
  ...
  if (reached_polish_solution_threshold_)
    dq_limit = settings_.polish_dq_limit;
...
void Pik::initializeSolver(const std::vector<IkTask>& ik_tasks, const Eigen::VectorXd& q_initial_guess)
{
