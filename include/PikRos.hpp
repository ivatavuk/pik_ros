...
struct IkTask
{
   ...
   */
  IkTask(IkTaskType t_type, const std::string& t_frame_name, const Eigen::VectorXd& t_desired_value,
...
struct Settings
{
  ...
  double polish_gradient_threshold =
      1e-2; /**< When sum of task gradients reaches this threshold solution polishing starts */
...
public:
   ...
   */
  Pik(ros::NodeHandle& nh, const std::string& robot_name, const std::string& joint_model_group_name,
...
private:
  ...
  Eigen::Vector3d getOrientationErrorRPY(Eigen::Quaterniond desired_quat, Eigen::Quaterniond current_quat,
  ...
  void initializeSolver(const std::vector<IkTask>& ik_tasks, const Eigen::VectorXd& q_initial_guess);
